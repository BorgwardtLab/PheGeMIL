...
def main():
    ...
    for i in range(5):
        ...
        pd.DataFrame(np.array([index,y_test,y_pred]).T, columns=['gid','target_yield', 'predicted_yield']).to_csv(
                 os.path.join(output_path, f'results_lasso_split{i}.csv'), index=False)
