...
# THIS SCRIPT IS FOR Om_crop IMAGES
# ------------------------------------------
import numpy as np
import pandas as pd
import os
...
raw_data_path = os.path.join(DATA_FOLDER, 'uncompressed')
processed_data_path = os.path.join(DATA_FOLDER, 'numpy_MIL_resized', 'multispectral_images_Om_crop') 
...
for idx, row in tqdm(df_all.iterrows()):
    ...
    for folder in folders:
        # Get filenames
        folder_path = os.path.join(raw_data_path, folder, 'Om_crop')
        fname = os.path.join(folder_path, row['plot_id']+'.tif')
        if os.path.isfile(fname):
            # load the image 
            img, x, y = load_npy_img(fname)
...
df_all.to_csv(os.path.join(DATA_FOLDER, 'csv', 'df_20201006_numpy_MIL_npy_coordinates.csv'), index=False)