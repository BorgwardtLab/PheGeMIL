...
# each DEM image plotID and preprocess 
...
df_all = pd.read_csv(os.path.join(DATA_FOLDER, 'csv', 'df_20200119_numpy_demimg.csv'))

# Set up paths
input_path = os.path.join(DATA_FOLDER, 'numpy_MIL', 'height_images')
output_path = os.path.join(DATA_FOLDER, 'numpy_MIL_resized', 'height_images')
...
np_filenames = []
input_img_paths = input_path + '/' + df_all['Path'] + '/' + df_all['PlotID'].apply(str) + '.npy' #need to remove .tif
...
for idx_list in tqdm(bags_indices_list):
    ...
    output_filepath = os.path.join(output_path, str(row['PlotID'])[:-4]+'.npy')
...
df_new.to_csv(os.path.join(DATA_FOLDER, 'csv', 'df_20200119_numpy_demimg_resized.csv'), index=False)
