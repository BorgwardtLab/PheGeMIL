...
# For test time, only load relevant channels
# ------------------------------------------
...
class CropBagDataset(Dataset):
    ...
    def __init__(self, csv_path, base_path, transform=None, subsample_size=None,
        ...
        print(base_path)
        paths = self.df['Path'] + '/' if not any(self.df['Path'].isna()) else ''
        self.X = base_path + '/' + paths + self.df['Filename']
...
class GenotypeDataset(Dataset):
    ...
    def __init__(self, csv_path, base_path, split_name='train', split_id=0, 
                verbose=True, normalize=True, scaler_path=None, standardize_target_path=None):
...
class FusedFullBags(Dataset):
    ...
    def __init__(self, csv_path, base_path_multispectral, base_path_thermal, base_path_dem,
                ...
                verbose=True, resized=True, return_dates=False,
                ms_standardize_path=None, genotype_scaler_path=None, target_standardize_path=None):
        ...
        self.multispectral_dataset = CropBagDataset(csv_path, base_path_multispectral, transform=transform_multispectral, 
                                                    ...
                                                    return_dates=return_dates, standardize_path=ms_standardize_path,
                                                    standardize_target_path=target_standardize_path)
        if base_path_thermal is not None:
            ...
            self.thermal_dataset = None
        # Can use the same class for DEM images
        if base_path_dem is not None:
        ...
        if base_path_genotype is not None:
            self.genotype_dataset = GenotypeDataset(csv_path, base_path_genotype, split_name=split_name, split_id=split_id,
                                                    verbose=verbose, normalize=normalize_genotype, scaler_path=genotype_scaler_path)
        else:
            self.genotype_dataset = None
    ...
    def __getitem__(self, index):
        ...
        if self.return_dates:
            ...
            bag_thermal, _, dates_thermal = self.thermal_dataset[index] if self.thermal_dataset else None, None, None
            bag_dem, _, dates_dem = self.dem_dataset[index] if self.dem_dataset else None, None, None
            genotype, _ = self.genotype_dataset[index] if self.genotype_dataset else None, None
            ...
            bag_thermal, _ = self.thermal_dataset[index] if self.thermal_dataset else None, None
            bag_dem, _ = self.dem_dataset[index] if self.dem_dataset else None, None
            genotype, _ = self.genotype_dataset[index] if self.genotype_dataset is not None else None, None
...
def variable_length_collate_fullfusedbags(batch):
    ...
    for batch_element in batch:
        ...
        genotypes.append(geno[0]) # TODO FIX THIS
    ...
    for img_type in ['multispectral', 'thermal', 'dem']:
        if imgs_bags[img_type][0] is None: # set lengths to 0
            lengths[img_type] = [0 for i in imgs_bags[img_type]]
            extended_imgs_bags[img_type].append(imgs_bags[img_type])
        else:
    ...
    if return_dates: 
        ...
        return tensor(extended_imgs_bags['multispectral'], dtype=torch.float), \
            tensor(np.nan_to_num(np.array(extended_imgs_bags['thermal'], dtype=float)), dtype=torch.float), \
            tensor(np.nan_to_num(np.array(extended_imgs_bags['dem'], dtype=float)), dtype=torch.float), \
