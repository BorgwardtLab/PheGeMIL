...
from .modules import Aggregator, DeepGenoFusionMIL
...
from .data import FusedGenoBags, CropTransforms, variable_length_collate_fusedbags, get_transformations
...
class MILCropYieldGeno(pl.LightningModule):
    """Base class for a the MILCropYieldGeno model
    ...
    def training_step(self, data_batch, batch_i):
        ...
        if self.hparams.temporal_encoding:
            ...
            y_pred = self.forward((data_multispectral, lengths_multispectral, data_thermal, \
                                    lengths_thermal, data_genotype, dates_multispectral, dates_thermal))
        else:
            data_multispectral, data_thermal, data_genotype, labels, lengths_multispectral, lengths_thermal = data_batch
            y_pred = self.forward((data_multispectral, lengths_multispectral, data_thermal, lengths_thermal, data_genotype))
    ...
    def get_dataset(self, split):
        ...
        if fixed_size: # Usually the data is on /links/groups/borgwardt/Jesse_2018/numpy_MIL_resized
            ...
            base_path_multispectral = os.path.join(self.hparams.data_path, 'numpy_MIL_resized', '2017-2018_CIMMYT_Wheat')
            base_path_thermal = os.path.join(self.hparams.data_path, 'numpy_MIL_resized', 'thermal_images')
            base_path_genotype = os.path.join(self.hparams.data_path, 'numpy_MIL_resized', 'genotypes')
        ...
        if self.hparams.temporal_encoding:
            assert 'df_20200107_numpy_MIL_npy_coordinates_dates_genofiltered' in self.hparams.csv_name
        # base_path_thermal = os.path.join(self.hparams.data_path,'numpy_MIL_resized', 'thermal_images') #This is the original path
        dataset = FusedGenoBags(os.path.join(self.hparams.data_path,'csv', self.hparams.csv_name),
...
def get_parser():
    ...
    parent_parser = add_general_arguments(parent_parser, 'genotype_mil')
    parser = MILCropYieldGeno.add_model_specific_args(parent_parser)
    return parser
...
if __name__ == '__main__':
    ...
    if hyperparams.hypersearch:
        for trial in hyperparams.trials(hyperparams.n_trials):
            print(trial)
            run_fitting_experiment(MILCropYieldGeno, trial)
    else:
        run_fitting_experiment(MILCropYieldGeno, hyperparams)
