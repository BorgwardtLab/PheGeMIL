...
class MILCropYieldGenoFinetune(pl.LightningModule):
    ...
    def __init__(self, hparams):
        ...
        model1 = MILCropYieldGeno.load_from_metrics(
                        weights_path=os.path.join(self.original_folder,get_saved_checkpoint(self.original_folder)),
                        tags_csv=os.path.join(self.original_folder, 'meta_tags.csv'),
                        on_gpu=gpus)
        ...
        def forward_genotype(self, x_genotype):
            ...
            # Aggregate
            x = self.aggregator.modified_aggregation(x)
            
            batch_size, n_heads, L = x.size()
            x = x.view(batch_size, L*n_heads)
            return self.regressor(x)

        def modified_aggregation(self, x):
            ...
            return M
        model1.model.aggregator.modified_aggregation = MethodType(modified_aggregation, model1.model.aggregator)
    ...
    def add_model_specific_args(cls, parent_parser):
        ...
        parent_parser.add_argument('--original-experiment', default='temporal_thermal_best_params_cv_with_geno', type=str)
        parent_parser.add_argument('--original-version', default='0', type=str)
...
def get_parser():
    ...
    parent_parser = add_general_arguments(parent_parser, 'genotype_mil_finetuning')
