...
from .modules import Aggregator, FullFusionMIL
...
from .data import FusedFullBags, CropTransforms, variable_length_collate_fullfusedbags, get_transformations
...
class MILCropYieldFull(pl.LightningModule):
    """Base class for a the MILCropYieldFull model
    ...
    def __init__(self, hparams):
        ...
        # build model
        self.model = FullFusionMIL(multispectral=True, 
                    encoder_type_multispectral=self.hparams.encoder_type, 
                    thermal=not self.hparams.no_thermal, encoder_type_thermal=self.hparams.encoder_type_thermal,
                    thermal_image_size=(40,40), dem=not self.hparams.no_dem, encoder_type_dem=self.hparams.encoder_type_dem,
                    genotype=not self.hparams.no_genotype, genotype_size=40767,
    ...
    def training_step(self, data_batch, batch_i):
        ...
        if self.hparams.temporal_encoding:
            ...
            y_pred = self.forward((data_multispectral, lengths_multispectral, data_thermal, \
                                    lengths_thermal, data_dem, lengths_dem, data_genotype, dates_multispectral,\
                                        dates_thermal, dates_dem))
        else:
            data_multispectral, data_thermal, data_dem, data_genotype, labels, lengths_multispectral, lengths_thermal, \
                lengths_dem = data_batch
            y_pred = self.forward((data_multispectral, lengths_multispectral, data_thermal, lengths_thermal, data_dem,\
                lengths_dem, data_genotype))
    ...
    def get_dataset(self, split):
        ...
        # tmp_folder = '/tmp/matteos_cached_resized_images'
        tmp_folder = '/links/groups/borgwardt/Data/Jesse_2018/numpy_MIL_resized'
        
        if self.hparams.halve_training_images:
            base_path_multispectral = os.path.join(tmp_folder, 'half_cropped_ms_images')
        else:
            base_path_multispectral = os.path.join(tmp_folder, '2017-2018_CIMMYT_Wheat')
        base_path_thermal = os.path.join(tmp_folder, 'thermal_images')
        base_path_dem = os.path.join(tmp_folder, 'height_images')
        base_path_genotype = os.path.join(tmp_folder, 'genotypes')
        if self.hparams.normalize:
            standardize_path = os.path.join(self.hparams.data_path,'csv', 'df_20201108_normalization_features.csv')
        else:
    ...
    def add_dataset_specific_args(cls, parent_parser):  # pragma: no cover
        ...
        group.add_argument(
            '--csv-name', default='df_20200121_numpy_MIL_npy_coordinates_dates_genofiltered_dems.csv', type=str) # df_20190927_numpy_MIL_npy_coordinates.csv
        group.add_argument(
        ...
        parent_parser.add_argument('--augmentation', default='light', type=str,
    ...
    def add_model_specific_args(cls, parent_parser):
        ...
        # Training options
        parent_parser.opt_list('--bag-size', default=32, type=int,
                        options=[32, 16, 8], tunable=True,
                        help='Bag size is the maximum number of images per sample '
                        'across all nodes (can also be None --> use all images)')
        ...
        parent_parser.opt_list('--learning-rate', default=0.0001, type=float,
                        ...
                        tunable=True)
        parent_parser.add_argument('--halve-training-images', default=False, action='store_true',
                    help='Use the halved images for training on single beds.' \
                         'CAREFUL: this does not work with temporal encoding yet.')
        ...
        parent_parser.add_argument('--encoder-type-thermal', default='resnet', type=str,
                        choices=['convnet', 'resnet'])
        parent_parser.add_argument('--encoder-type-dem', default='resnet', type=str,
                        choices=['convnet', 'resnet'])
        parent_parser.opt_list('--normalize-genotype', default=False, 
                    action='store_true', tunable=True, options=[True, False])
        parent_parser.add_argument('--normalize', default=False, action='store_true',
                    help='Normalize input ms images using mean and std.')
        parent_parser.opt_list('--no-thermal', default=True, 
                    action='store_true', tunable=True, options=[True, False])
        parent_parser.opt_list('--no-genotype', default=False, 
                    action='store_true', tunable=True, options=[True, False])
        parent_parser.opt_list('--no-dem', default=False, 
                    action='store_true', tunable=True, options=[True, False])
        parent_parser.opt_list('--n-heads', default=1, type=int,
                        options=[1, 4, 8],
                        tunable=True)
        parent_parser.opt_list('--scheduler', default='plateau', type=str,
...
def get_parser():
    ...
    parent_parser = add_general_arguments(parent_parser, 'full_mil')
    parser = MILCropYieldFull.add_model_specific_args(parent_parser)
    return parser
...
if __name__ == '__main__':
    ...
    if hyperparams.hypersearch:
        for trial in hyperparams.trials(hyperparams.n_trials):
            print(trial)
            run_fitting_experiment(MILCropYieldFull, trial)
    else:
        run_fitting_experiment(MILCropYieldFull, hyperparams)
